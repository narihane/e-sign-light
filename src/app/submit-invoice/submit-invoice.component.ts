...
import { Document, InvoiceDocument, InvoiceLine } from '../shared/_models/AE-invoice.model';
...
export class SubmitInvoiceComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private appService: AppService,
    private codeService: CodesService, private invoiceService: InvoiceService,
  ...
  onInvoiceAdd() {
    ...
    this.invoiceForm.get('items')?.value.forEach(element => {
      ...
      var itemType = '';
      var itemCode = '';
      this.codeService.searchCodes(1, 10, element.category).subscribe((data) => {
        ...
        this.issuerService.getIssuer().subscribe(data => {
          ...
          var invoice: Document = {
            ...
            receiver: {
              ...
              type: this.invoiceForm.get("fixed")?.get('receiverType')?.value == 'business' ? 'B' :
                (this.invoiceForm.get("fixed")?.get('receiverType')?.value == 'person' ? 'P' : 'F'),
              address: {
                "country": "EG",
                "governate": "Egypt",
